...
export function buildHealthDeclarationPDF(
  ...
  try {
    ...
    // SIGNATURE SECTION MOVED UP - Place it before medical notes
    // Signature Section
    ...
    if (healthDeclaration.signature) {
      try {
        ...
        // Add the signature image - centered horizontally and positioned immediately after the section header
        pdf.addImage(
          healthDeclaration.signature,
          'PNG',
          (pageWidth / 2) - (maxSignatureWidth / 2), // Center horizontally
          currentY,  // Position at current Y (right after section header)
          maxSignatureWidth,
          signatureHeight
        );
        
        // Add more space after signature
    ...
    // Medical Notes Section (moved after signature)
