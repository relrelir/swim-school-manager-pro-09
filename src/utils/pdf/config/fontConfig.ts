...
/**
 * Configure Hebrew font support for PDF documents
 */
...
/**
 * Apply font fallback strategy when primary font fails
 */
export const applyFontFallback = (pdf: jsPDF): void => {
  try {
    // Try to use helvetica as fallback
    pdf.setFont('helvetica');
    console.log('Using fallback font: helvetica');
  } catch (fontError) {
    console.error('Font fallback also failed:', fontError);
    // If all else fails, use whatever default font is available
  }
};
